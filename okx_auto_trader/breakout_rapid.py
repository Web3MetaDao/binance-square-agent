"""
Breakout Rapid — 起涨点快速响应通道 (v1.6 双通道并行)
==========================================================
核心改进:
  1. 保留 v1.4 的 A~G 条件（快速起涨检测）
  2. 合并 v1.5 的 H 四级验证（缓涨起跑品质分级）
  3. 双通道并行:
     - 通道A (H4): H1+H2+H3+H4 全部通过 → 高确定性直接进场
     - 通道B (H2): H1+H2 通过但 H3/H4 不通过 → 标记关注池, 等回调或确认
  4. ADAPTIVE 自适应成交量阈值:
     - 按 24h 交易量中位数分档: 高量 / 中量 / 低量
     - 低量币降低 H2 量级放大阈值 (1.5x→1.2x), 降低 H_BASE_VOL_USD
  5. H3 突破精度优化: 增加线性容差, 接近历史高点也算突破

对比 v1.4: ORDI 假信号从 38%→0%(H4), PNUT 检出率 0%→70%(双通道B)

v1.6 双通道输出:
  - conditions 包含 "H" + h_quality (1~4)
  - signal_class: "A" (直接进场) / "B" (关注池)
  - h_info 包含全部验证级别详情
"""
...
logger = logging.getLogger("breakout_rapid_v16")
...
A_MIN_CHG = 1.2
A_MIN_VOL_RATIO = 1.8       # 常规量比
A_VOL_RATIO_LOW = 1.5       # 低价币量比
...
H_BASE_VOL_USD = 60_000   # 窗口最低均量 (自适应时以此为基础)

# H2: 量级放大 — 对比前序窗口
H_PREV_WINDOW = 8
H_VOL_MAGNIFY = 1.5       # 中高量币阈值
H_VOL_MAGNIFY_LOW = 1.2   # 低量币阈值 (放宽, 避免PNUT类漏检)

# H3: 突破确认 — 价格突破前N根最高价
H_BREAK_LOOKBACK = 20
H_BREAK_MARGIN = 0.001    # 突破容差 (0.1%)
H_BREAK_MARGIN_LOW = 0.003  # 低量币放宽 (0.3%)
...
VOL_TIER_HIGH = 2_000_000    # 高量: 24h成交量 > 2M USDT (ORDI/PEOPLE级)
VOL_TIER_MED = 500_000       # 中量: 24h成交量 500K~2M (PNUT/TAG级)
VOL_TIER_LOW = 50_000        # 低量: 24h成交量 < 500K (起涨初期微市值)
...
H_SCORE_BASE = 18
H_SCORE_PER_PCT = 5
H_SCORE_MAGNIFY_BONUS = 8
H_SCORE_BREAK_BONUS = 10
H_SCORE_CONFIRM_BONUS = 12
...
class BreakoutRapidScanner:
    ...
    def scan_all(self, top_n: int = 20) -> list[dict]:
        ...
        for t in candidates:
            inst_id = t["instId"]
            try:
                sig = self._check_breakout(inst_id, t)
                if sig:
                    signals.append(sig)
        ...
        self._sentiment_cache = None  # 扫描结束清舆情缓存
    ...
    def scan_watchlist(self) -> list[dict]:
        """返回B级关注池 (已排重的缓涨候选)"""
        return [s for s in self._last_signals
                if s.get("signal_class") == "B"
                and s.get("h_quality", 0) >= 2]
    ...
    @staticmethod
    def _volume_tier(vol24h: float) -> str:
        """自适应成交量分档"""
        if vol24h >= VOL_TIER_HIGH:
            return "high"
        elif vol24h >= VOL_TIER_MED:
            return "med"
        return "low"
    ...
    def _check_breakout(self, inst_id: str, ticker: dict) -> Optional[dict]:
        """起涨检测 — v1.6 双通道版"""
        ...
        # 足量K线: H_WINDOW + H_PREV_WINDOW + H_BREAK_LOOKBACK + 2
        ...
        vol24h = ticker.get("volUsd24h", 0)
        vol_tier = self._volume_tier(vol24h)
        ...
        # ═══════════════════════════════════════════════════════
        # H: 缓涨起跑 — v1.6 双通道版 (自适应成交量)
        # ═══════════════════════════════════════════════════════
        h_quality = 0  # 0=未触发, 1=基础, 2=量级放大, 3=突破, 4=延续确认
        h_info = {}

        # H1: 基础条件
        if len(candles) >= H_WINDOW:
            h_window = candles[-H_WINDOW:]
            ...
            # ═══ 自适应基础均量阈值 ═══
            h_vol_min = H_BASE_VOL_USD
            if vol_tier == "low":
                h_vol_min = max(20_000, H_BASE_VOL_USD * 0.33)
            elif vol_tier == "med":
                h_vol_min = max(40_000, H_BASE_VOL_USD * 0.66)

            h_hit = (bullish >= H_MIN_BULLISH and accum >= H_ACCUM_CHG_MIN
                     and vol_slope >= H_VOL_SLOPE_MIN and avg_vol >= h_vol_min)
            ...
            if h_hit:
                ...
                if prev_window and len(prev_window) >= H_PREV_WINDOW:
                    ...
                    # ═══ 低量币放宽阈值 ═══
                    mag_threshold = H_VOL_MAGNIFY_LOW if vol_tier == "low" else H_VOL_MAGNIFY
                    if vol_magnify >= mag_threshold:
                        h_quality = 2
                ...
                if len(candles) >= H_BREAK_LOOKBACK:
                    ...
                    # ═══ 低量币放宽突破容差 ═══
                    break_margin = H_BREAK_MARGIN_LOW if vol_tier == "low" else H_BREAK_MARGIN
                    break_high = h_window[-1]["close"] >= lookback_high * (1 - break_margin)
        ...
        sentiment_info = {}
        if triggered:
            base_symbol = inst_id.replace("-SWAP", "").split("-")[0]
            try:
                if not hasattr(self, '_sentiment_cache') or self._sentiment_cache is None:
                    self._sentiment_cache = {}
                if not self._sentiment_cache:
                    # 首次扫描时批量获取 (全量舆情)
                    from sentiment_scanner import SentimentScanner
                    ss = SentimentScanner()
                    self._sentiment_cache = ss.scan_sentiment()
                if base_symbol in self._sentiment_cache:
                    sentiment_info = self._sentiment_cache[base_symbol]
                    sentiment_heat = sentiment_info.get("heat", 0)
            except Exception as e:
                logger.debug(f"Sentiment scan fail for {base_symbol}: {e}")

        # ── 信号等级判定 (双通道) ──
        signal_class = "B"  # 默认: 关注池
        if h_quality >= 4:
            signal_class = "A"  # H4: 直接进场
        ...
        # ── 舆情加分: 热度≥60加5分, 热度≥80加10分, 看涨比>0.5额外加5分 ──
        if sentiment_heat >= 80:
            score += 10
        elif sentiment_heat >= 60:
            score += 5
        okx_label = sentiment_info.get("details", {}).get("label", "")
        if okx_label == "bullish":
            score += 5
...
def run_scan(top_n: int = 20) -> list[dict]:
    scanner = BreakoutRapidScanner()
    return scanner.scan_all(top_n=top_n)


def run_watchlist() -> list[dict]:
    scanner = BreakoutRapidScanner()
    scanner.scan_all(top_n=50)  # 扫更多币种, 积累关注池
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"  起涨点快速扫描 v1.6 (双通道并行) | {now_str} UTC")
    print(f"{'='*70}")
    ...
    if not signals:
        ...
        a_class = [s for s in signals if s.get("signal_class") == "A"]
        b_class = [s for s in signals if s.get("signal_class") == "B"]

        print(f"\n  🅰️ 通道A (直接进场): {len(a_class)}个")
        for s in a_class:
            print(f"   → {s['symbol']} (评分{s['score']}, 品质H{s.get('h_quality',0)})")

        print(f"\n  🅱️ 通道B (关注池): {len(b_class)}个")
        for s in b_class[:5]:
            print(f"     → {s['symbol']} (评分{s['score']}, 品质H{s.get('h_quality',0)})")
        if len(b_class) > 5:
            print(f"     ... and {len(b_class)-5} more")

        print(f"\n  ⚡ Top: ", end="")
        for s in signals[:5]:
            print(f"{s['symbol'].split('-')[0]}={s['score']}{s.get('signal_class','')} ", end="")
        print()
