...
REFERRAL_KEYWORDS = ["返佣", "邀请码", "注册", "开户", "手续费", "优惠"]
...
def generate_danmu_reply(
    ...
    if category == "referral":
        context_parts.append(f"返佣链接: {REFERRAL_LINK}")
...
返佣链接（需要时提供）：{REFERRAL_LINK}

回复规则：
1. 直接称呼用户名 "{username}"
2. 回答控制在60-100字
3. 如果问合约，必须加一句风险提示
4. 如果问返佣，给出链接并说明最高20%返佣
5. 语气自然，像真人主播说话
"""
...
def _fallback_reply(danmu_text: str, username: str, market_report: dict) -> str:
    ...
    if category == "contract":
        return f"{username}，合约高风险！建议仓位不超过总资金20%，做好止损。想开户享返佣可以用我的链接：{REFERRAL_LINK}"

    if category == "referral":
        return f"{username}，返佣链接在这：{REFERRAL_LINK} ，通过这个链接注册可以享受最高20%手续费返佣，省钱！"
